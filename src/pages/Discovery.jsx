...
import { Search, Loader2, ChevronRight, CheckCircle2, Circle, Sparkles } from 'lucide-react'
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500">Enter an investment theme — AI finds the most exposed public companies</p>
      </div>
      ...
      <div className="flex gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="e.g. Small Modular Reactors, AI Data Centers, Longevity..."
        ...
        <button
          ...
          className="px-6 py-3.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-xl shadow-sm transition-colors flex items-center gap-2 min-w-[180px] justify-center"
        >
          {loading
            ? <><Loader2 className="w-4 h-4 animate-spin" /> Researching...</>
            : <><Sparkles className="w-4 h-4" /> Find Companies</>
          }
        </button>
      </div>
        ...
        {EXAMPLES.map(ex => (
          <button key={ex} onClick={() => setTheme(ex)}
            className={clsx(
              ...
              theme === ex ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300 hover:text-slate-900'
            )}
          >
            {ex}
          </button>
      ...
      {loading && (
        <div className="flex flex-col items-center py-16 text-slate-400">
          ...
          <p className="text-sm font-medium">Asking GPT-4o-mini to research "{theme}"...</p>
          <p className="text-xs mt-1">Finding 10–15 companies with direct exposure</p>
        </div>
      )}
      ...
      {!loading && companies.length > 0 && (
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">{companies.length} companies found</h2>
              <p className="text-sm text-slate-500">{selectedCount} selected · click to toggle</p>
            </div>
              ...
              return (
                <div key={c.ticker} onClick={() => toggleCompany(c.ticker)}
                  className={clsx(
                    ...
                    selected ? 'bg-white border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md' : 'bg-slate-50 border-slate-200 opacity-50'
                  )}
                >
                  <div className="absolute top-3 right-3">
                    {selected ? <CheckCircle2 className="w-5 h-5 text-blue-500" /> : <Circle className="w-5 h-5 text-slate-300" />}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className={clsx('px-2 py-0.5 rounded-full text-xs font-medium', sectorColor(c.sector))}>{c.sector}</span>
                    <span className={clsx('px-2 py-0.5 rounded-full text-xs font-semibold capitalize', EXPOSURE_COLORS[c.exposure] || EXPOSURE_COLORS.secondary)}>{c.exposure}</span>
                  </div>
                  <p className="mt-2 text-slate-500 text-xs leading-relaxed line-clamp-3">{c.rationale}</p>
                </div>
              )
            })}
          </div>
